function QRCodeScanner(options) {
  ...
  const controlsOverlayElement = c(
    "div",
    {
      className: "controls",
    },
  ...
  const videoPanel = c(
    ...
    [videoElement, controlsOverlayElement]
  );
  ...
  function retrieveVideoInputs() {
    ...
    navigator.mediaDevices.enumerateDevices().then(function (devices) {
      availableVideoInputs = devices.filter(function (device) {
        return device.kind === "videoinput";
      });
      switchVideoInputButton.style.display =
        availableVideoInputs.length < 2 ? "none" : "block";
    });
  }
  ...
  function handleSwitchVideoInputButtonClick() {
    ...
    const newAvailableVideoInputIndex =
      (currentAvailableVideoInputIndex + 1) % availableVideoInputs.length;
  ...
  appendChildren(rootElement, [errorPanel, loadingPanel, videoPanel]);
